...
@interface MHZipArchive ()
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime;
@end


@implementation MHZipArchive {
    NSString *_path;
    NSString *_filename;
    mh_zipFile _zip;
}
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error delegate:(id<MHZipArchiveDelegate>)delegate {
    // Begin opening
    mh_zipFile zip = mh_unzOpen((const char*)[path UTF8String]);
    if (zip == NULL) {
        NSDictionary *userInfo = [NSDictionary dictionaryWithObject:@"failed to open zip file" forKey:NSLocalizedDescriptionKey];
        if (error) {
            *error = [NSError errorWithDomain:@"MHZipArchiveErrorDomain" code:-1 userInfo:userInfo];
        }
        return NO;
    }
    ...
    mh_unz_global_info  globalInfo = {0ul, 0ul};
    mh_unzGetGlobalInfo(zip, &globalInfo);
    
    // Begin unzipping
    if (mh_unzGoToFirstFile(zip) != UNZ_OK) {
        ...
        if (error) {
            *error = [NSError errorWithDomain:@"MHZipArchiveErrorDomain" code:-2 userInfo:userInfo];
        }
        return NO;
    }
    ...
    do {
        @autoreleasepool {
            if ([password length] == 0) {
                ret = mh_unzOpenCurrentFile(zip);
            } else {
                ret = mh_unzOpenCurrentFilePassword(zip, [password cStringUsingEncoding:NSASCIIStringEncoding]);
            }
            ...
            mh_unz_file_info fileInfo;
            memset(&fileInfo, 0, sizeof(mh_unz_file_info));
            
            ret = mh_unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            if (ret != UNZ_OK) {
                success = NO;
                mh_unzCloseCurrentFile(zip);
                break;
            }
            ...
            mh_unzGetCurrentFileInfo(zip, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
            ...
            if (nil != err) {
                NSLog(@"[MHZipArchive] Error: %@", err.localizedDescription);
            }
            ...
            if(!fileIsSymbolicLink)
            {
                ...
                while (fp) {
                    int readBytes = mh_unzReadCurrentFile(zip, buffer, 4096);
                ...
                if (fp) {
                    ...
                    if (fileInfo.dosDate != 0) {
                        ...
                        if (attr) {
                            if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
                                ...
                                NSLog(@"[MHZipArchive] Failed to set attributes - whilst setting modification date");
                            }
                        }
                    }
                    ...
                    if (permissions != 0) {
                        ...
                        if ([fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil] == NO) {
                            ...
                            NSLog(@"[MHZipArchive] Failed to set attributes - whilst setting permissions");
                        }
            ...
            else
            {
                ...
                int bytesRead = 0;
                while((bytesRead = mh_unzReadCurrentFile(zip, buffer, 4096)) > 0)
                {
                    buffer[bytesRead] = 0;
    ...
    for (NSDictionary * d in directoriesModificationDates) {
        if (![[NSFileManager defaultManager] setAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[d objectForKey:@"modDate"], NSFileModificationDate, nil] ofItemAtPath:[d objectForKey:@"path"] error:&err]) {
            NSLog(@"[MHZipArchive] Set attributes failed for directory: %@.", [d objectForKey:@"path"]);
        }
        if (err) {
            NSLog(@"[MHZipArchive] Error setting directory file modification date attribute: %@",err.localizedDescription);
        }
    }
...
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths {
    BOOL success = NO;
    MHZipArchive *zipArchive = [[MHZipArchive alloc] initWithPath:path];
    if ([zipArchive open]) {
...
- (BOOL)open {
    ...
    _zip = mh_zipOpen([_path UTF8String], APPEND_STATUS_CREATE);
    return (NULL != _zip);
}


- (void)zipInfo:(mh_zip_fileinfo*)zipInfo setDate:(NSDate*)date {
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(NSString *)fileName {
    ...
    mh_zipOpenNewFileInZip(_zip, afileName, &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION);
    ...
    while (!feof(input))
    {
        ...
        mh_zipWriteInFileInZip(_zip, buffer, len);
    }
    
    mh_zipCloseFileInZip(_zip);
    free(buffer);
    fclose(input);
    return YES;
}
...
- (BOOL)writeData:(NSData *)data filename:(NSString *)filename {
    ...
    mh_zip_fileinfo zipInfo = {{0,0,0,0,0,0},0,0,0};
    [self zipInfo:&zipInfo setDate:[NSDate date]];
    
    mh_zipOpenNewFileInZip(_zip, [filename UTF8String], &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION);
    
    mh_zipWriteInFileInZip(_zip, data.bytes, (unsigned int)data.length);
...
- (BOOL)close {
    NSAssert((_zip != NULL), @"[MHZipArchive] Attempting to close an archive which was never opened");
    mh_zipClose(_zip, NULL);
    return YES;
}
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime {
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[MHZipArchive] MSDOS date masks don't add up");
